...
---
### 참고
- 파이썬에서 리스트 슬라이싱을 할 때 끝 인덱스에 리스트의 길이를 벗어나는 인덱스를 지정해도 오류가 발생하지 않음
  ```
  my_list = [1, 2, 3, 4, 5]

  # 리스트의 길이를 벗어나는 끝 인덱스 사용
  print(my_list[1:10])  # 출력: [2, 3, 4, 5]

  # my_list[1:10]은 10이 리스트의 길이를 벗어나지만, 파이썬은 자동으로 리스트의 마지막  
  # 요소까지만 슬라이싱, 따라서 [2, 3, 4, 5]가 출력
  ```
